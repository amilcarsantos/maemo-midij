...
//SE_DEL//#include "controlobjectthread.h"
...
QMutex MidiScriptEngine::m_scriptEngineLock;
...
MidiScriptEngine::~MidiScriptEngine() {
    ...
    // to signals.
    //SE_DEL//m_connectedControls.clear();
...
/*SE_DEL//    QList<ConfigKey> keys = m_controlCache.keys();
...
bool MidiScriptEngine::safeExecute(QString function, QString data) {
    qDebug() << QString("MidiScriptEngine: Exec2 Thread ID=%1").arg((ulong)QThread::currentThreadId(),0,16);
...
double MidiScriptEngine::getValue(QString group, QString name) {
    ...
    /*SE_DEL//ControlObjectThread *cot = getControlObjectThread(group, name);
    if (cot == NULL) {*/
    	double value; // ++MidiJ
    	value = m_valuesMap.value(group + name);
        qDebug() << "MidiScriptEngine: getValue " << group << name;
        return value;
/*SE_DEL//    }
...
void MidiScriptEngine::setValue(QString group, QString name, double newValue) {
    ...
    // qDebug() << QString("----------------------------------MidiScriptEngine: SetValue Thread ID=%1").arg((ulong)QThread::currentThreadId(),0,16);
    ...
    qDebug() << "Warning: script setValue [" << group << "," << name
             << "] value:" << newValue;
    m_valuesMap.insert(group + name, newValue); //++MidiJ
}
...
/*SE_DEL//    ControlObject* cobj = ControlObject::getControl(ConfigKey(group,name));
