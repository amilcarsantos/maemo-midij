...
//	qDebug() << "GraphicsMidiOutputItem mouseMoveEvent";
...
//
//void GraphicsMidiControlItem::propagateEvent(QGraphicsSceneMouseEvent* event)
//{
//	// FIXME ver se o setAceptTouchEvent() chega pro telele (ou se tb necessida do setAceptedMouseButtons(NoButtons)
//
//	QList<QGraphicsItem*> itemList = scene()->items(event->lastScenePos(), Qt::IntersectsItemShape, Qt::AscendingOrder);
//	foreach (QGraphicsItem* item, itemList)
//	{
//		if (item == this)
//			continue;
//		// don't propagate event to background NOR items at the same level (zOrder)
//		if ((item->zValue() > 0) && (item->zValue() < zValue()) )
//		{
