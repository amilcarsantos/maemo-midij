...
class NetMIDI(Dependence):

    def configure(self, build, conf):
        build.env.Append(CPPDEFINES = '__NETMIDI__')

    def sources(self, build):
        build.env.Uic4('dlgprefnetcastdlg.ui')
        return ['midi/netmidienumerator.cpp',
                'midi/midideviceudp.cpp',
                'engine/engineheadsidechain.cpp',
                'engine/enginenetcastudp.cpp',
                'dlgprefnetcast.cpp'
               ]
...
class MixxxCore(Feature):
    ...
    def depends(self, build):
        return [SoundTouch, KissFFT, ReplayGain, PortAudio, PortMIDI, Qt,
                FidLib, SndFile, FLAC, OggVorbis, OpenGL, TagLib, NetMIDI]
