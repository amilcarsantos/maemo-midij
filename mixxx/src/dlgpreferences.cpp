...
#ifdef __NETMIDI__
    m_wnetcast = new DlgPrefNetcast(this, config);
    addPageWidget(m_wnetcast);
#endif
    setupMidiWidgets();
...
#endif
#ifdef __NETMIDI__
    connect(this, SIGNAL(showDlg()), m_wnetcast, SLOT(slotUpdate()));
    connect(buttonBox, SIGNAL(accepted()), m_wnetcast,SLOT(slotApply()));
#endif
...
#ifdef __NETMIDI__
    m_pNetcastButton = new QTreeWidgetItem(contentsTreeWidget, QTreeWidgetItem::Type);
    m_pNetcastButton->setIcon(0, QIcon(":/images/preferences/ic_preferences_broadcast.png"));
    m_pNetcastButton->setText(0, tr("Headphone Broadcast"));
    m_pNetcastButton->setTextAlignment(0, Qt::AlignLeft | Qt::AlignVCenter);
    m_pNetcastButton->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled);
#endif
...
#endif
#ifdef __NETMIDI__
    } else if (current == m_pNetcastButton) {
           pagesWidget->setCurrentWidget(m_wnetcast->parentWidget()->parentWidget());
#endif
