...
#endif
#ifdef __NETMIDI__
    m_pNMEnumerator = new NetMidiEnumerator();
#endif
}
...
void MidiDeviceManager::saveMappings(bool onlyActive) {
    ...
    while (it.hasNext())
    {
        ...
        QString ofilename = name.right(name.size()-name.indexOf(" ")-1).replace(" ", "_").replace("/", "_");
...
#endif
#ifdef __NETMIDI__
        m_devices.append(m_pNMEnumerator->queryDevices());
#endif
    }
