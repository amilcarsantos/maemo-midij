...
StantonSCS3d.spinningPlatter = false;    // Spinning platter LEDs // --MidiJ
...
StantonSCS3d.loopModeLeds = {   1:{ 0x48:[0x61, 0x62, 0x63], 0x4A:[0x61, 0x62, 0x63], 0x4C:[0x60, 0x5f, 0x5e], 0x4E:[0x60, 0x5f, 0x5e],
                                    0x4F:[0x66, 0x67, 0x68], 0x51:[0x66, 0x67, 0x68], 0x53:[0x69, 0x6a, 0x6b], 0x55:[0x69, 0x6a, 0x6b] }, // Auto loop mode
                                2:{ 0x48:[0x61, 0x62, 0x63], 0x4A:[0x61, 0x62, 0x63], 0x4C:[0x60, 0x5f, 0x5e], 0x4E:[0x60, 0x5f, 0x5e],
                                    0x4F:[0x66, 0x67, 0x68], 0x51:[0x66, 0x67, 0x68], 0x53:[0x69, 0x6a, 0x6b], 0x55:[0x69, 0x6a, 0x6b] }, // Manual loop mode
                                3:{ 0x48:[0x61, 0x62, 0x63], 0x4A:[0x61, 0x62, 0x63], 0x4C:[0x60, 0x5f, 0x5e], 0x4E:[0x60, 0x5f, 0x5e],
                                    0x4F:[0x66, 0x67, 0x68], 0x51:[0x66, 0x67, 0x68], 0x53:[0x69, 0x6a, 0x6b], 0x55:[0x69, 0x6a, 0x6b] } };
...
// ++MidiJ --- Init values ---------
StantonSCS3d.initValue = [      ["[Master]","volume",   0.51],
                                ["[Channel1]","volume", 0.51],
                                ["[Channel2]","volume", 0.61],
                                ["[Flanger]","lfoDepth", 0.51],
                                ["[Flanger]","lfoDelay", 5170],
                                ["[Flanger]","lfoPeriod", 1010000],
                                ["[Channel1]","filterLow", 1],
                                ["[Channel1]","filterMid", 1],
                                ["[Channel1]","filterHigh", 1],
                                ["[Channel2]","filterLow", 1],
                                ["[Channel2]","filterMid", 1],
                                ["[Channel2]","filterHigh", 1]
                          ];
...
//     midi.sendSysexMsg(StantonSCS3d.sysex.concat([0x02, StantonSCS3d.channel, 0xF7]),7);
    ...
    // ++MidiJ
    for (i=0; i<StantonSCS3d.initValue.length; i++) {
        var group = StantonSCS3d.initValue[i][0];
        var name = StantonSCS3d.initValue[i][1];
        var initValue = StantonSCS3d.initValue[i][2];
        engine.setValue(group, name, initValue);
    }
    // MidiJ
...
StantonSCS3d.pitch = function (channel, control, value) {   // Lower the sensitivity of the pitch slider
    ...
    StantonSCS3d.pitchLEDs(newValue); // ++MidiJ
}
...
StantonSCS3d.gain = function (channel, control, value) {
    ...
    StantonSCS3d.gainLEDs(value/127); // ++MidiJ
}
...
//        }
            StantonSCS3d.playLED(1); // ++MidiJ
        return;
    }
    StantonSCS3d.modifier["play"]=0;
    StantonSCS3d.playLED(0); // ++MidiJ
}
...
StantonSCS3d.B13 = function (channel, control, value, status) {
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        if (currentMode == "vinyl3" || currentMode == "vinyl")
            midi.sendShortMsg(byte1,control,0x01); // Make button red
    }
    else {
...
//     if (StantonSCS3d.modeSurface[modeName] != StantonSCS3d.modeSurface[currentMode])    // If different,
    ...
    switch (modeName) {    // Prep LEDs for certain modes
        ...
        case "loop2":
        case "loop3":
                var redButtonLEDs = [0x48, 0x49, 0x4a, 0x4c, 0x4d, 0x4e, 0x4f, 0x50, 0x51, 0x53, 0x54, 0x55];
                if ((currentMode.substring(0,4) != "loop") || (currentMode.substring(0,4) != "trig") || StantonSCS3d.state["changedDeck"]) {
...
StantonSCS3d.S3absolute = function (channel, control, value) {
    ...
    switch (currentMode) {
        case "fx":
        	...
        	StantonSCS3d.FXDepthLEDs(engine.getValue("[Flanger]","lfoDepth")); //++MidiJ
        	break;
        case "eq":
        	var v = script.absoluteNonLin(value,0,1,4);
        	engine.setValue("[Channel"+StantonSCS3d.deck+"]","filterLow",v);
        	StantonSCS3d.EQLowLEDs(v); //++MidiJ
        	break;
    }
}
...
StantonSCS3d.S4absolute = function (channel, control, value) {
    ...
    switch (currentMode) {
        case "fx": 
        	script.absoluteSlider("[Flanger]","lfoDelay",value,50,10000); 
        	StantonSCS3d.FXDelayLEDs(engine.getValue("[Flanger]","lfoDelay")); //++MidiJ
        	break;
        case "eq": 
        	var v = script.absoluteNonLin(value,0,1,4);
        	engine.setValue("[Channel"+StantonSCS3d.deck+"]","filterMid",v);
        	StantonSCS3d.EQMidLEDs(v);
        	break;
...
StantonSCS3d.S5absolute = function (channel, control, value) {
    ...
    switch (StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"]) {
        case "fx": 
			script.absoluteSlider("[Flanger]","lfoPeriod",value,50000,2000000);
        	StantonSCS3d.FXPeriodLEDs(engine.getValue("[Flanger]","lfoPeriod")); //++MidiJ
			break;
        case "eq":
        	var v = script.absoluteNonLin(value,0,1,4);
        	engine.setValue("[Channel"+StantonSCS3d.deck+"]","filterHigh", v);
        	StantonSCS3d.EQHighLEDs(v);
        	break;
    }
}
...
StantonSCS3d.C1touch = function (channel, control, value, status) {
    ...
    if ((status & 0xF0) == 0x90) {    // If button down
        ...
        switch (currentMode) {
            case "vinyl":
            	midi.sendShortMsg(byte1,0x62,0x00); // Turn off C1 lights // ++MidiJ
            	break;
            case "vinyl2":
...
StantonSCS3d.S3touch = function () {
    ...
    if (StantonSCS3d.modifier[currentMode]==1){
        switch (currentMode) {
            case "fx":
				engine.setValue("[Flanger]","lfoDepth",0.5);
				
				break;
            case "eq":
				engine.setValue("[Channel"+StantonSCS3d.deck+"]","filterLow",1);
    ...
    if (StantonSCS3d.modifier[currentMode]==1){ // If the current mode button is held down, reset the control to center
        switch (currentMode) {
            case "fx":
            	...
            	break;
            case "eq": 
            	engine.setValue("[Channel"+StantonSCS3d.deck+"]","filterMid",1);
				StantonSCS3d.EQMidLEDs(1); //++MidiJ
            	break;
        }
    }
    if ((status & 0xF0) == 0x90) {    // If button down
...
StantonSCS3d.S4buttonLights = function (channel, light, button) {     // Turn on/off button lights for multiple buttons
	if (StantonSCS3d.debug) print("S4buttonLights() button:" + button + " light: " + light ); // ++MidiJ
    var marker;
    var byte1 = 0x90 + channel;
    var color=0x00; // Off
    if (light) color=0x01;  // On
...
StantonSCS3d.SurfaceButton = function (channel, control, value, status) {
    ...
    if ((status & 0xF0) != 0x80) {    // If button down
//--MidiJ         midi.sendShortMsg(byte1,control,0x01); // Turn on button lights
//--MidiJ         midi.sendShortMsg(byte1,StantonSCS3d.buttonLEDs[control],0x01);
        switch (currentMode) {
	        case "trig":
	        case "trig2":
	        case "trig3":
            ...
            case "loop3":
            	// Manual loop
            	var leds = StantonSCS3d.loopModeLeds[index][control];
                for (i=0; i<leds.length; i++)
                    midi.sendShortMsg(byte1,leds[i],0x1); // light LEDs ON
...
//                     break;
//                 }
//--MidiJ                if (StantonSCS3d.pitchPoints[index][control] == -0.1)
//--MidiJ                    StantonSCS3d.pitchPoints[index][control] = engine.getValue("[Channel"+StantonSCS3d.deck+"]","rate");
//--MidiJ                else {
//--MidiJ                    // Need 100% range for values to be correct
//--MidiJ                    engine.setValue("[Channel"+StantonSCS3d.deck+"]","rateRange",1.0);
//--MidiJ                    engine.setValue("[Channel"+StantonSCS3d.deck+"]","rate",StantonSCS3d.pitchPoints[index][control]); 
//--MidiJ                }
                break;
/*--MidiJ
            case "trig":
        ...
        switch (currentMode) {
            ...
            case "loop3":
            	...
            	var leds = StantonSCS3d.loopModeLeds[index][control];
                for (i=0; i<leds.length; i++)
                    midi.sendShortMsg(byte1,leds[i],0x0); // light LEDs OFF
				return;
		}
...
//--MidiJ    midi.sendShortMsg(byte1,marker,0x00); // Turn off activated button LED
    midi.sendShortMsg(byte1,marker,0x41); // ++MidiJ
...
// Router Util methods
StantonSCS3d.currentDeckMode = function () {
	return StantonSCS3d.mode_store["[Channel"+StantonSCS3d.deck+"]"];
}