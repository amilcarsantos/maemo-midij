...
//SE_DEL//#include <qapplication.h>
//SE_DEL//#include "defs_version.h"
//SE_DEL//#include "widget/wwidget.h"    // FIX-ME: This should be xmlparse.h
#include "MixxxControl.h"
#include "MidiMessage.h"
//SE_DEL//#include "midiinputmappingtablemodel.h"
//SE_DEL//#include "midioutputmappingtablemodel.h"
#include "MidiMapping.h"
//SE_DEL//#include "midiledhandler.h"
...
#endif
    //SE_DEL//    m_pMidiInputMappingTableModel = new MidiInputMappingTableModel(this);
    //SE_DEL//    m_pMidiOutputMappingTableModel = new MidiOutputMappingTableModel(this);
}
...
bool MidiMapping::isMixxxControlMapped(const MixxxControl& control) {
    m_mappingLock.lock();
...
/*SE_DEL//    QStringList commandLineArgs = QApplication::arguments();
...
void MidiMapping::loadPreset(QString path) {
	...
	//SE_DEL//    loadPreset(WWidget::openXMLFile(path, "controller"));
}
...
//--MidiJ    Q_ASSERT(!m_mappingLock.tryLock());
    // Create a new blank DomNode
    //SE_DEL//    QString blank = "<MixxxMIDIPreset schemaVersion=\"" + QString(XML_SCHEMA_VERSION) + "\" mixxxVersion=\"" + QString(VERSION) + "+\">\n"
    QString blank = "<MixxxMIDIPreset schemaVersion=\"1\" mixxxVersion=\"1+\">\n"
    "</MixxxMIDIPreset>\n";
    QDomDocument doc("Bindings");
    doc.setContent(blank);
//--MidiJ    m_Bindings = doc.documentElement();
    loadPreset(doc.documentElement(), false); // ++MidiJ
}
 ...
 void MidiMapping::buildDomElement() {
    ...
    //Iterate over all of the command/control pairs in the input mapping
     ...
     while (it.hasNext()) {
         ...
         addControl(controlNode, wtfbbqdevicename); //FIX-ME: Remove this device shit until we have multiple device support.
     }
     ...
     while (outIt.hasNext()) {
         ...
         outIt.value().serializeToXML(outputNode, true);
          ...
          //Add the control node we just created to the XML document in the proper spot
         addOutput(outputNode, wtfbbqdevicename); //FIX-ME: Remove this device shit until we have multiple device support.
     }
 }
*/
...
{
    //TO-DO: Check if mapping already exists for this MidiMessage.

    //Add to the input mapping.
    m_inputMapping.insert(message, control);
    return true; //XX-X is this right? should this be returning whether the add happened successfully?
...
// (See Control*::GetMidiValue())
 ...
 // static
/*SE_DEL//double MidiMapping::ComputeValue(MidiOption midioption, double _prevmidivalue, double _newmidivalue)
{
    double tempval = 0.;
    double diff = 0.;
    ...
    else if (midioption == MIDI_OPT_DIFF)
    {
        ...
        //Apply sensitivity to signed value. FIX+ME
       // if(sensitivity > 0)
