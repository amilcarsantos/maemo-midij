	...
	: MidiButton(midiMsg)
{
	...
	if (!sliderNode.isNull())
	{
		...
		// use slider position or button on/off (default)
		m_buttonSliderValue = sliderNode.attribute(CONTROL_SLIDER_NOTEVALUE, "") == QString(CONTROL_SLIDER);
		m_nudge = GuiUtils::toIntBound(sliderNode.attribute(CONTROL_SLIDER_NUDGE), 0, 30, m_nudge);
		toMinMax(sliderNode.text(), m_minimum, m_maximum);
	}
}
...
void MidiSliderAbsolute::toMinMax(const QString& text, int& min, int& max)
{
	if (text.contains(','))
	{
	...
	max = GuiUtils::toIntBound(text, min + 1, 127, max);
}
...
int MidiSliderRelative::midiByte2(EventType type, QPointF pos)
{
	...
	case Move:
		...
		m_midiByte2 = MidiSliderAbsolute::midiByte2(Move, pos);
		if (m_midiByte2 == lastMidiByte2)
			return -1; // skip equal values...
	...
	: MidiButton(midiMsg)
{
	...
	QDomElement wheelNode = controlNode.firstChildElement(CONTROL_WHEEL);
	if (!wheelNode.isNull())
	{
		m_reverse = wheelNode.attribute(CONTROL_WHEEL_REVERSE, "false") == QString("true");
		// use wheel position or button on/off (default)
		m_buttonWheelValue = wheelNode.attribute(CONTROL_WHEEL_NOTEVALUE, "") == QString(CONTROL_WHEEL);
		MidiSliderAbsolute::toMinMax(wheelNode.text(), m_minimum, m_maximum);
	}
}
...
{
	m_deltaUBound = m_maximum - ((m_maximum - m_minimum)/10);
	m_deltaLBound = m_minimum - ((m_maximum - m_minimum)/10);;
}
...
int MidiWheelRelative::midiByte2(EventType type, QPointF pos)
{
	...
	case Pressed:
		...
		return MidiWheelAbsolute::midiByte2(type, pos);
	case Move:
		lastMidiByte2 = m_midiByte2;
		m_midiByte2 = MidiWheelAbsolute::midiByte2(Move, pos);
		if (m_midiByte2 == lastMidiByte2)
		...
		if (deltaByte2 > m_deltaUBound) //(m_maximum - 10%))
			deltaByte2 = deltaByte2 - (m_maximum - m_minimum + 1);
		else
			if (deltaByte2 < m_deltaLBound) //(m_minimum - 10%))
