...
#else
	m_socket->writeDatagram(datagram.data(), datagram.size(), m_targetAddress, m_targetPort);
#endif
}
...
#else
	m_socket->writeDatagram((const char*)data, length, m_targetAddress, port);
#endif
}
...
void MidiObjectUdp::setParams(Params* pParams)
{
	if (pParams == NULL)
		return;

	QHostAddress testAddress;
	if (!pParams->targetIp.isEmpty() && testAddress.setAddress(pParams->targetIp))
	{
		m_targetAddress = testAddress;
	}
	else
	{
		m_targetAddress = QHostAddress(QHostAddress::Broadcast);
		// invalid IP
		pParams->targetIp.clear();
	}
	qDebug(QString("MidiObjectUdp - TargetIP: %1").arg(m_targetAddress.toString()).toAscii());
}
